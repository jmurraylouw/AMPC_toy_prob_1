...
for index = 1:length(N_train_list) % Loop through N_train_list
    ...
    %             end
                ...
                if (sum(abs(eig(A_tilde)) > 1) ~= 0) % If tilde eigenvalues are still unstable
                    ...
                    search_space(N_train_i, q_i, p_i) = NaN; 
